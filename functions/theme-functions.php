<?php
	...
	* Author: Cole Geissinger
	 ...
	 * Author: Cole Geissinger
	/*****/
	function mms_get_call_to_action_slider() {
	...
	********************************************************************/
	 ...
	 * Author: Cole Geissinger
	/*****/
	function mms_add_placeholder_field($position, $form_id) {
		if($position == 25) { ?>
	...
	add_action("gform_field_standard_settings", "mms_add_placeholder_field", 10, 2);


	/*****
	 * Description: Set our JavaScripts needed for the mms_add_placeholder_field in Gravity Forms
	 * Since: 0.1
	 * Author: Cole Geissinger
	/*****/
	function mms_add_placeholder_editor_js() { ?>
		<script>
	...
	<?php }
	add_action('gform_editor_js', 'mms_add_placeholder_editor_js');
	 ...
	 * Author: Cole Geissinger
	/*****/
	function mms_gform_enqueue_scripts($form, $is_ajax = false) { ?>
		<script>
			jQuery(function() {
				<?php
	...
	<?php }
	add_action('gform_enqueue_scripts', 'mms_gform_enqueue_scripts', 10, 2);
	...
	/*****
	 * Description: Display a title with an icon to the left of it
	 * Useage: [icon type="tools" text="Tools" /]
	 * Parameters: type="$icon_name"  | Accepts tools, schools, creativity, or community
	 *					text="$title_here" | Insert the text you wish to display
	 * Since: 1.2
	 * Author: Cole Geissinger
	/*****/
	function mms_icon_title($atts, $content = null) {
		extract(shortcode_atts(array(
			'type' => '',
			'text' => ''
		), $atts));
		...
		if(!empty($text)) {
			return '<span class="icons-' . $type . '"></span> ' . $text;
		}
	}
	...
	function mms_register_shortcodes() {
		...
		add_shortcode('icon', 'mms_icon_title');
